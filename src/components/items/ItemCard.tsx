...
export function ItemCard({ item, members, hideDone, hideNotRelevant, onUpdateItem, onDeleteItem, onChangeQuantity, onUpdateMemberState, dragHandleProps, isDraggable = true, itemTextWidth = ITEM_TEXT_WIDTH_MIN, itemTextWidthMode = 'auto', compactRowPageMinWidthPx = 0, expandSignal = 0, collapseSignal = 0, categoryNames, categoryOrder, onClearAddItemDraft, itemNameFontClassName = 'text-lg leading-snug', itemNameFontStep = ITEM_NAME_FONT_DEFAULT, isOfflineActionsDisabled = false, allowItemMutationQueue = false, tourTargetsEnabled = false }: ItemCardProps) {
  ...
  const rowClipOverflow = !isEditing
      ...
      >
        ...
        <div
          className={
            compactRow
              ? compactFixedLayout
                ? `box-border flex min-w-full flex-nowrap items-center whitespace-nowrap ${itemRowFlexGapClassName} ${itemRowHorizontalPaddingClassName}${rowClipOverflow ? ' overflow-hidden' : ''}`
                : `box-border flex w-max flex-nowrap items-center whitespace-nowrap ${itemRowFlexGapClassName} ${itemRowHorizontalPaddingClassName}${rowClipOverflow ? ' overflow-hidden' : ''}`
              : `box-border flex min-h-0 items-center whitespace-nowrap ${itemRowFlexGapClassName} ${itemRowHorizontalPaddingClassName}${showMenu && rowClipOverflow ? ' overflow-hidden' : ''}`
          }
          style={{
