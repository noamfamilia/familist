...
export function Modal({
  ...
  const closeBtn = !hideClose && (
    <button
      type="button"
      onClick={onClose}
      className={`${closeBtnClass} text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-2xl leading-none p-1 rounded hover:bg-gray-100 dark:hover:bg-slate-600`}
  ...
  const panelClass = fullScreenMobile
    ? `relative mx-auto w-full bg-white dark:bg-slate-800 shadow-lg dark:shadow-slate-900/50 outline-none animate-in fade-in zoom-in-95 duration-200 max-sm:fixed max-sm:inset-0 max-sm:m-0 max-sm:flex max-sm:h-[100dvh] max-sm:max-h-[100dvh] max-sm:max-w-none max-sm:flex-col max-sm:overflow-hidden max-sm:rounded-none max-sm:p-0 sm:relative sm:mx-auto sm:my-4 sm:max-h-[calc(100vh-2rem)] sm:overflow-y-auto sm:rounded-xl sm:p-6 sm:p-8 sm:shadow-lg ${sizeClasses[size]} ${contentClassName}`
    : `relative mx-auto my-4 bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-slate-900/50 p-6 sm:p-8 w-full max-h-[calc(100vh-2rem)] overflow-y-auto outline-none ${sizeClasses[size]} animate-in fade-in zoom-in-95 duration-200 ${contentClassName}`
