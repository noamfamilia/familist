...
export function ListCard({ list, existingListNames, onUpdate, onDelete, onArchive, onDuplicate, onLeave, onRefresh, dragHandleProps, labels = [], onUpdateLabel }: ListCardProps) {
        ...
        <div className="px-3 py-2 bg-gray-50 dark:bg-slate-900 group-hover:bg-gray-100 dark:group-hover:bg-slate-700 transition-colors border-t border-gray-200 dark:border-slate-600 space-y-2">
          ...
          {onUpdateLabel && !editingComment && !isRenaming && (
            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
              <svg className="h-4 w-4 text-gray-500 dark:text-gray-400 flex-shrink-0" viewBox="0 0 1024 1024" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              ...
              <select
                value={list.label || ''}
                onChange={(e) => void onUpdateLabel(list.id, e.target.value)}
                className="text-sm bg-transparent border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1 text-gray-700 dark:text-gray-300 focus:outline-none focus:border-teal cursor-pointer"
              >
