...
export function ListCard({ list, existingListNames, onUpdate, onDelete, onArchive, onDuplicate, onLeave, dragHandleProps, labels = [], onUpdateLabel, onSelectLabel, currentFilter = 'Any', onClearCreateInput, onClearCreateInputIfTyped }: ListCardProps) {
  ...
  return (
    <>
    {/* Main card content */}
    <div className="group bg-gray-50 dark:bg-list-charcoal rounded-lg hover:bg-gray-100 dark:hover:bg-list-charcoal-hover transition-colors">
      {/* Card row */}
      ...
      <div className="flex-1 min-w-0 relative">
        {list.userArchived ? (
          ...
          <span
            className="flex items-center gap-1 font-medium text-lg text-primary dark:text-list-name hover:text-teal cursor-pointer"
            data-tour="list-card"
            onClick={(e) => {
              e.stopPropagation()
              setNewName(list.name)
          ...
          <Link
            href={`/list/${list.id}`}
            className="block font-medium truncate text-lg text-primary dark:text-list-name hover:text-teal"
            data-tour="list-card"
          >
            {list.name}
            {ownerBadge}
      ...
      {menuOpen && (
        <div className="px-3 py-2 bg-gray-50 dark:bg-list-charcoal group-hover:bg-gray-100 dark:group-hover:bg-list-charcoal-hover transition-colors space-y-2 rounded-b-lg">
