...
export function ListsView({ viewMode, homeTourSteps, showTutorial = true, inviteToken = null, onInviteHandled, selectedLabel = 'Any', onLabelsChange, onSelectLabel }: ListsViewProps) {
  ...
  const createLabelDropdownRef = useRef<HTMLDivElement>(null)
  const [createAddingLabel, setCreateAddingLabel] = useState(false)
  const [createNewLabelText, setCreateNewLabelText] = useState('')
  const createAddLabelInputRef = useRef<HTMLInputElement>(null)
  const createAddLabelPopoverRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    onLabelsChange?.(labels)
  }, [labels, onLabelsChange])

  // Pre-set label when user starts typing (non-join mode)
  const showCreateLabel = !!inputValue && !inputValue.startsWith('@')
  useEffect(() => {
    if (showCreateLabel) {
      setNewListLabel(selectedLabel === 'Any' || selectedLabel === '' ? '' : selectedLabel)
    }
  }, [showCreateLabel, selectedLabel])

  // Outside-click for create label dropdown
  useEffect(() => {
    if (!createLabelDropdownOpen) return
    const close = (e: MouseEvent) => {
      if (createLabelDropdownRef.current && !createLabelDropdownRef.current.contains(e.target as Node)) {
        e.preventDefault()
        e.stopPropagation()
  ...
  useEffect(() => {
    if (createAddingLabel && createAddLabelInputRef.current) {
      createAddLabelInputRef.current.focus()
    }
  }, [createAddingLabel])

  // Outside-click for create add-label popover
  useEffect(() => {
    if (!createAddingLabel || createLabelDropdownOpen) return
    const handleMouseDown = (e: MouseEvent) => {
      if (createAddLabelPopoverRef.current && !createAddLabelPopoverRef.current.contains(e.target as Node)) {
        e.preventDefault()
        e.stopPropagation()
        document.addEventListener('click', (ce) => { ce.preventDefault(); ce.stopPropagation() }, { capture: true, once: true })
        handleCreateAddLabelDone()
      }
    }
  ...
  const handleCreateAddLabelDone = () => {
    const trimmed = createNewLabelText.trim()
    if (trimmed) {
      setNewListLabel(trimmed)
    }
    setCreateAddingLabel(false)
    setCreateNewLabelText('')
  }

  const handleCancelCreateAddLabel = () => {
    setCreateAddingLabel(false)
    setCreateNewLabelText('')
  }
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (!target || formWrapperRef.current?.contains(target)) return
      clearCreateInput()
    }
      ...
      <div ref={formWrapperRef} className="relative">
        {showCreateLabel && (
          <div
            ...
            className="absolute bottom-full left-0 right-0 mb-1 rounded-lg border border-gray-200 dark:border-slate-600 shadow-lg z-20 bg-white dark:bg-slate-800 overflow-visible"
          >
            <div className="p-3">
              <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Set Label</div>
              <div className="relative" ref={createLabelDropdownRef}>
                <button
                  type="button"
                  onClick={() => { setCreateLabelDropdownOpen(o => !o); setCreateAddingLabel(false); setCreateNewLabelText('') }}
                  className="text-sm bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-md px-2 py-1 text-gray-700 dark:text-gray-300 focus:outline-none focus:border-teal cursor-pointer flex items-center gap-1 w-full"
                >
                  ...
                  {newListLabel || <span className="text-gray-400">None</span>}
                  <svg className={`h-3 w-3 ml-auto transition-transform ${createLabelDropdownOpen ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                    <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                </button>
                {createLabelDropdownOpen && (
                  <div className="absolute left-0 bottom-full mb-1 min-w-[140px] w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-lg dark:shadow-slate-900/50 z-50 overflow-hidden">
                    {labels.map(l => (
                      <button
                        key={l}
                        type="button"
                        onClick={() => { setNewListLabel(l); setCreateLabelDropdownOpen(false) }}
                        className={`w-full text-left px-3 py-1.5 text-sm transition-colors ${
                          newListLabel === l ? 'bg-teal/10 text-teal font-semibold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700'
                        }`}
                      >
                        {l}
                      </button>
                    ...
                    <button
                      type="button"
                      onClick={() => { setNewListLabel(''); setCreateLabelDropdownOpen(false) }}
                      className={`w-full text-left px-3 py-1.5 text-sm transition-colors ${
                        !newListLabel ? 'bg-teal/10 text-teal font-semibold' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-slate-700'
                      }`}
                    >
                      None
                    </button>
                    <button
                      type="button"
                      onClick={() => { setCreateLabelDropdownOpen(false); setCreateAddingLabel(true) }}
                ...
                {createAddingLabel && !createLabelDropdownOpen && (
                  <div
                    ref={createAddLabelPopoverRef}
                    className="absolute left-0 bottom-full mb-1 z-50 bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-600 shadow-lg p-2 min-w-[160px]"
                  >
                    <div className="relative">
                      <input
                        ref={createAddLabelInputRef}
                        type="text"
                        value={createNewLabelText}
                        onChange={(e) => setCreateNewLabelText(e.target.value)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') { e.preventDefault(); handleCreateAddLabelDone() }
                          if (e.key === 'Escape') handleCancelCreateAddLabel()
                        }}
