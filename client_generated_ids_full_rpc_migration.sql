-- Full RPC migration for client-generated UUID support (offline-first).
-- Backward compatible for existing clients via DEFAULT NULL parameters.
--
-- Updates included:
--   1) create_list          -> optional p_id
--   2) bulk_add_list_items  -> optional positional p_item_ids[]
--   3) import_list          -> optional p_id, optional positional p_item_ids[]
--   4) duplicate_list       -> optional p_id, optional positional p_item_ids[], optional p_target_member_id
...
begin
  ...
  -- Insert list with optional client id.
...
drop function if exists public.import_list(text, text, text, jsonb, uuid, uuid[]);
...
begin
  ...
  -- Bulk insert items with optional positional client ids.
  if v_len > 0 then
...
grant execute on function public.import_list(text, text, text, jsonb, uuid, uuid[]) to authenticated;
...
begin
  ...
  -- Duplicate source items; optionally enforce client ids positionally by stable ordering.
