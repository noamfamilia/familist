...
3. **Import in the app**  
   Sign in, open **Import from Google Sheet** (link on the home lists screen), paste the full browser URL of the sheet, optionally set a **List name**, then **Import**.
...
5. **List title**  
   If `GOOGLE_SHEETS_API_KEY` is set in the server environment, the new list may be named after the spreadsheet. Otherwise the app uses **Import**, **Import 2**, etc., unless you enter a name in **List name (optional)**.
